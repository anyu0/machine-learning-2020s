...
Here, parameter $\theta_i$ is updated with value $\theta_{t+1, i}$ at time $t+1$. $G_t, ii$ is a diagonal matrix with each diagonal term being the sum of the squares of all past gradients. $\epsilon$ is a smoothing term to prevent division by zero.
...
\begin{align*}
	E[g^t]_t &= \gamma E[g^t]_{t-1} + (1-\gamma)g^2_t\\

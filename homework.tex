...
\subsection*{(iii)}
	...
	For the decision tree, we set aside 10\% of the training data as a validation set. To improve runtime and reduce noise, we consider only high variance features (features with the top 5\% variance). We built 50 trees of depths, 5, 10, ..., 250 and ran the validation set on them. The best prediction accuracy came from the tree with 15 layers, which classified the validation set with an accuracy of 0.8796.
	...
	The best classifier is by far the Naive Bayes classifier, which outperforms both KNN and the 

...
\subsection*{(iii)}
	...
	\begin{claim}
		$\hat{\mu_n} = \frac{1}{n} \sum_{i=1}^n X_i $ is a consistent and unbiased estimator for $\mu$.
	\end{claim}
	\begin{proof}
		By Law of Large Numbers, 
		$\hat{\mu_n} $ converges to $\hat{\mu} = \lim\limits_{n\rightarrow \infty} \frac{1}{n} \sum_{i=1}^n X_i$,
		so $\hat{\mu_n}$ is consistent.\\
		Since $\mathbb{E}[\hat{\mu_n}]
